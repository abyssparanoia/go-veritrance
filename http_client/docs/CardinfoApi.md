...
> AccountResponse PaynowidV1GetCardinfo(ctx).Params(params).AuthHash(authHash).Execute()
...
func main() {
    ...
    configuration := openapiclient.NewConfiguration()
    apiClient := openapiclient.NewAPIClient(configuration)
    resp, r, err := apiClient.CardinfoApi.PaynowidV1GetCardinfo(context.Background()).Params(params).AuthHash(authHash).Execute()
    if err != nil {
